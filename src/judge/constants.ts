...
/**
 * 규칙표 버전. **규칙표가 바뀌면 올린다** — 경계값, 감점, 그리고 보류 규칙까지.
 * 과거 판정과 섞이지 않게 하는 표시다.
 *
 * 처음에는 "경계값이나 감점이 바뀌면"이라고만 적었는데, v1.1.0은 그 둘을 그대로 두고
 * 보류 규칙(H7)만 더했다. 기준은 무엇을 고쳤는가가 아니라 **같은 입력이 다른 결과를
 * 내는가**다.
 */
...
export const STANCE = {
   ...
   *
   * **더 굽은 쪽 무릎**에 건다. A2는 덜 굽은 쪽으로 채점하므로 재는 대상이 서로 다르고,
   * 그래서 이 값이 A2의 경계(kneeAngleBoundaries)와 같은 160이어도 순환이 생기지 않는다.
   * 양쪽 무릎 모두에 걸면 두 가지가 한꺼번에 깨진다 —
   *  (1) 한쪽만 굽힌 비대칭 주춤서기가 감점이 아니라 '자세 아님'이 된다. A2·A3는
   *      바로 그 자세를 감점하려고 만든 항목인데 그 앞에서 판정이 멈춘다.
   *  (2) 구간 안의 모든 프레임이 양쪽 ≤160°가 되므로 그 중앙값도 ≤160°다.
   *      = **A2의 major 밴드(> 160°)에 어떤 입력으로도 도달할 수 없게 된다.**
   * 한쪽이라도 굽었으면 "주춤서기를 시도했다"로 보고, 얼마나 못 굽혔는지와 얼마나
   * 비대칭인지는 A2·A3가 감점으로 말한다. 보류는 '자세 아님'에만 쓴다.
   */
  ...
  /**
   * 위 조건을 함께 만족한 구간이 이 시간 이상 이어져야 "멈춘 적이 있다"로 본다 (초).
   *
   * 문턱을 한 프레임으로 두면 앉았다 곧바로 일어나는 사람 — 한 번도 자세를 유지하지
   * 않은 사람 — 이 지나가는 길에 스친 한 프레임으로 점수를 받는다. 30fps에서
   * 0.2초는 7프레임이다. 한두 프레임은 내려가는 도중일 수 있어도 7프레임이 연속으로
   * 멈춰 있으면 의도적으로 멈춘 것이다.
   *
   * **A5의 minHoldSeconds(0.8초)와는 다른 목적이다.** 0.8초는 "충분히 유지했는가"를
   * 묻는 채점 경계이고(미달은 0.1 감점), 이 값은 "멈춘 적이 있는가"를 묻는 구간 탐지
   * 상수다(없으면 채점하지 않는다). A5의 경계를 여기에 그대로 쓰면 감점으로 말해야 할
   * 것을 보류가 다시 말하게 되고, 채점 경계가 구간을 정하는 순환도 생긴다.
   *
   * 다른 구간 탐지 상수와 마찬가지로 ε를 두지 않는다. H4의 ε는 등급이 갈리는
   * **채점 경계**를 보호하는 장치이고, 구간 탐지는 등급을 직접 만들지 않는다.
   */
  settledMinSeconds: 0.2,
  /**
