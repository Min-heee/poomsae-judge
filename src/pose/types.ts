...
export interface PoseSequence {
  ...
  originNote?: string;
  /**
   * 이 시퀀스의 `t = 0` 에 해당하는 `performance.now()` 시각. **실시간 입력에만 있다.**
   *
   * 파일에서 온 시퀀스에는 없고, 없는 것이 맞다 — 녹화 파일에 벽시계를 붙이면 재생할
   * 때마다 다른 값이 된다. 웹캠 훅만 채우고, 실시간 프레임을 **다른 시계 위로 옮겨야
   * 하는 곳**(게임의 기록기)에서만 읽는다.
   *
   * 없으면 받는 쪽은 "배치를 받은 시각"으로 되짚을 수밖에 없는데, 그 시각은 프레임이
   * 찍힌 시각보다 배치 지연 L 만큼 늦다. 그 상수 편향이 **시작 신호 직전 L ms 의
   * 프레임을 시작 이후로 밀어 넣는다** — 준비 자세가 채점되는 경로가 거기서 열린다.
   */
  epochMs?: number;
}
