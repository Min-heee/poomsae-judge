...
정직하게 적습니다. 아래는 이 실사에서 **확인하지 못했고**, 구현 중에 실측해야 합니다.

- **`worldLandmarks`의 축 방향과 부호**(y가 위인지 아래인지, z 부호). 빈 이미지로는 포즈가 0개라 확인 불가였습니다. 첫 실제 검출에서 찍어 보고 6.3절 부호를 확정하세요.
- **`landmarks`와 `worldLandmarks`의 각도 일치도.** 어느 쪽으로 규칙을 세울지 정하려면 실제 포즈에서 두 방식의 무릎각을 비교해 봐야 합니다.
- **실제 사람의 앞차기에서 lite 모델의 발목·발끝 안정성.** B3(높이)·B6(축발 흔들림)이 발 랜드마크에 의존하는데, 빠른 동작에서 발이 흐려지면 lite로는 불안정할 수 있습니다. 불안정하면 `full`(9.0MB)로 올리는 판단이 필요합니다.
- 모바일 사파리에서의 WASM·getUserMedia 동작.
- GitHub Pages 실배포(저장소가 아직 비공개이고 push는 메인 세션 몫).
